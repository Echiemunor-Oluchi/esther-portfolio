...
export function VideoEmbed({ url, title, platform }: VideoEmbedProps) {
  ...
  if (platform === 'TikTok' && tiktokId) {
    return (
      <div className="relative w-full aspect-[9/16] max-w-xs mx-auto rounded-none overflow-hidden bg-charcoal">
        <blockquote
  ...
  if (platform === 'Instagram' && instagramId) {
    return (
      <div className="relative w-full max-w-sm mx-auto bg-charcoal overflow-hidden">
        <blockquote
...
function ExternalVideoCard({
  ...
  return (
    <a
      ...
      className="group block relative overflow-hidden bg-charcoal border border-white/10 hover:border-accent/50 transition-colors duration-300"
      data-cursor="WATCH"
    >
      {/* Placeholder visual */}
      <div className="aspect-[9/16] max-w-xs w-full mx-auto flex flex-col items-center justify-center gap-4 p-6 relative">
        ...
        <div className="relative z-10">
          {platform === 'TikTok' ? (
            <svg viewBox="0 0 24 24" fill="white" className="w-12 h-12 opacity-30 group-hover:opacity-70 transition-opacity duration-300">
        ...
        {/* Play button */}
        <div className="relative z-10 w-14 h-14 rounded-full border border-accent/50 flex items-center justify-center group-hover:border-accent group-hover:bg-accent/10 transition-all duration-300">
        ...
        {title && (
          <div className="relative z-10 text-center">
            <p className="text-white/60 text-xs tracking-[0.1em] uppercase font-medium">{title}</p>
          </div>
        )}

        {/* Watch link */}
        <div className="relative z-10 flex items-center gap-2 text-accent text-xs tracking-[0.1em] uppercase font-semibold mt-2">
